...
final class NestopiaGameController {
    private weak var engine: NestopiaEngine?
    private let connectionChanged: (Bool) -> Void
    private var observers: [NSObjectProtocol] = []
    private var boundControllers: [GCController] = []

    init(engine: NestopiaEngine, connectionChanged: @escaping (Bool) -> Void) {
        self.engine = engine
    ...
    private func releaseButtons(player: Int) {
        for button in NestopiaControllerButton.allCases {
