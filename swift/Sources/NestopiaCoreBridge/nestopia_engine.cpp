...
namespace {
void setError(NestopiaEngine *engine, const char *message)
{
...
NestopiaEngine *nestopia_engine_create(const char *databasePath)
{
    auto engine = std::make_unique<NestopiaEngine>();
...
void nestopia_engine_destroy(NestopiaEngine *engine)
{
    ...
    nestopia_engine_unload_rom(engine);
...
bool nestopia_engine_load_rom(NestopiaEngine *engine, const char *romPath, const char *batteryPath)
{
    if (engine == nullptr || romPath == nullptr) return false;
    if (engine->loaded) nestopia_engine_unload_rom(engine);
...
void nestopia_engine_unload_rom(NestopiaEngine *engine)
{
    ...
    nestopia_engine_reset_inputs(engine);
}

bool nestopia_engine_is_loaded(const NestopiaEngine *engine) { return engine != nullptr && engine->loaded; }
...
const uint32_t *nestopia_engine_video_buffer(const NestopiaEngine *engine) { return engine == nullptr ? nullptr : engine->videoBuffer.data(); }
size_t nestopia_engine_video_pixel_count(void) { return videoWidth * videoHeight; }
const int16_t *nestopia_engine_audio_buffer(const NestopiaEngine *engine) { return engine == nullptr ? nullptr : engine->audioBuffer.data(); }
size_t nestopia_engine_audio_sample_count(const NestopiaEngine *engine) { return engine == nullptr ? 0 : engine->audioSamples; }
uint32_t nestopia_engine_audio_sample_rate(void) { return audioSampleRate; }
...
void nestopia_engine_reset(NestopiaEngine *engine, bool hardReset)
{
