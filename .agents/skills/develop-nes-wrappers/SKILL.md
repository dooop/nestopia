...
2. Put emulator behavior and shared state in `swift/Sources/NestopiaCoreBridge/`.
3. Put only marshaling and buffer conversion in `android/nestopia/src/main/cpp/nestopia_jni.cpp`.
...
```text
nestopia_engine.h <-> nestopia_engine.cpp
    |-> Swift NestopiaEngine -> SwiftUI/public Apple API
    `-> JNI -> NativeNestopia -> Kotlin NestopiaEngine -> Compose/public Android API
```
