...
@Composable
internal fun NestopiaExternalControllerInput(
    engine: NestopiaEngine,
    modifier: Modifier = Modifier,
) {
    AndroidView(
        factory = { context -> NestopiaControllerInputView(context).also { it.engine = engine } },
...
private class NestopiaControllerInputView(
    context: Context,
) : View(context),
    InputManager.InputDeviceListener {
    var engine: NestopiaEngine? = null
    private val inputManager = context.getSystemService(InputManager::class.java)
    private val devicePlayers = mutableMapOf<Int, Int>()
    private val directions = mutableMapOf<Int, Set<NestopiaButton>>()
    ...
    override fun onGenericMotionEvent(event: MotionEvent): Boolean {
        ...
        val pressed =
            buildSet {
                if (horizontal < -AXIS_THRESHOLD) add(NestopiaButton.Left)
                if (horizontal > AXIS_THRESHOLD) add(NestopiaButton.Right)
                if (vertical < -AXIS_THRESHOLD) add(NestopiaButton.Up)
                if (vertical > AXIS_THRESHOLD) add(NestopiaButton.Down)
            }
    ...
    private fun handleKey(
        ...
        val button = keyCode.toNestopiaButton() ?: return false
    ...
    private fun releasePlayer(player: Int) {
        NestopiaButton.entries.forEach { button -> engine?.setButton(button, false, player) }
    }

    private companion object {
        const val AXIS_THRESHOLD = 0.5f
        val DIRECTION_BUTTONS = setOf(NestopiaButton.Up, NestopiaButton.Down, NestopiaButton.Left, NestopiaButton.Right)
        ...
        fun Int.toNestopiaButton(): NestopiaButton? =
            when (this) {
                KeyEvent.KEYCODE_DPAD_UP -> NestopiaButton.Up
                KeyEvent.KEYCODE_DPAD_DOWN -> NestopiaButton.Down
                KeyEvent.KEYCODE_DPAD_LEFT -> NestopiaButton.Left
                KeyEvent.KEYCODE_DPAD_RIGHT -> NestopiaButton.Right
                KeyEvent.KEYCODE_BUTTON_A, KeyEvent.KEYCODE_BUTTON_Y -> NestopiaButton.A
                KeyEvent.KEYCODE_BUTTON_B, KeyEvent.KEYCODE_BUTTON_X -> NestopiaButton.B
                KeyEvent.KEYCODE_BUTTON_START -> NestopiaButton.Start
                KeyEvent.KEYCODE_BUTTON_SELECT -> NestopiaButton.Select
                else -> null
            }
    }
}
